...
| `guide.html` | 使い方ガイド（IVS の仕組み、コピーした字が化ける理由、Canva・Office での使い方、FAQ）。静的なページで、ツールの「使い方」とフッターからリンクする |
| `src/css/style.css` | デザイントークン（藍・金茶・朱）、ダーク/ライト、レスポンシブ |
| `src/css/guide.css` / `src/js/guide.js` | 使い方ガイドだけで使うスタイル / 目次の今いる節の表示（テーマの切り替えは `ui/theme.js` と共通） |
...
- `search-index.json`（約2.2MB）: 起動時に読み込む軽量インデックス
  ...
  - `extraRadicals`: 一覧表で2つ目以降に定義されている部首（UCS → 2つ目以降の部首番号の配列。例: `"3427": [3, 6]`。部首名検索や2つ目以降の部首での絞り込みに使用）
